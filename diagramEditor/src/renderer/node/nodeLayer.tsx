...
export function NodeLayer(props: NodeLayerProps) {
	const { nodeWidgetMap, portWidgetMap } = props;
	const nodes = useDiagramState(state => state.diagram.nodes);

	return (
		<StackingLayer>
			{Object.values(nodes).map(node => (
				<AbstractNodeWidget key={node.id} id={node.id} nodeWidgetMap={nodeWidgetMap} portWidgetMap={portWidgetMap} />
			))}
		</StackingLayer>
	);
}
