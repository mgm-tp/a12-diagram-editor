...
export function getCanvasDimensions(canvasId: string): Rectangle {
	const canvasElement = document.getElementById(canvasId);
	assertExists(canvasElement, 'Element mit ID "canvas" nicht gefunden');

	const { width, height } = canvasElement.getBoundingClientRect();
	return { width, height };
}

export function getCanvasPosition(canvasId: string): Point {
	const canvasElement = document.getElementById(canvasId);
	assertExists(canvasElement, 'Element mit ID "canvas" nicht gefunden');
