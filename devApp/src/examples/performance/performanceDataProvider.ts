...
export const performanceDataProvider: DataProvider = {
	name: "performanceDataProvider",
	canHandle(config) {
		return config.dataHolder.descriptor.diagram === "performance";
	},
	*provideData(config) {
		const { activityId } = config;
		if (config.operation === "load") {
			const state = createDiagramState({ gridStepSize: 5 }, performanceDiagram as Diagram);
