...
import simpleDiagram from "../../../resources/simpleDiagram.json";

export const basicDataProvider: DataProvider = {
	name: "basicDataProvider",
	canHandle(config) {
		return config.dataHolder.descriptor.diagram === "basic";
	},
	*provideData(config) {
		const { activityId } = config;
		if (config.operation === "load") {
			const state = createDiagramState({ gridStepSize: 5 }, simpleDiagram as Diagram);
			yield* put(ActivityActions.setData({ activityId, data: state }));
		}
	}
};
